...
int ei_main(int argc, char** argv)
{
	...
	char*		button_title		= "Toujours pas...";
	...
	static char* button_filename = "misc/klimt.jpg";

	static ei_surface_t		button_image			= NULL;
	static ei_anchor_t img_anchor = ei_anc_north;



	/* Create the application and change the color of the background. */
	ei_app_create(&screen_size, EI_FALSE);
	button_image = hw_image_load(button_filename, ei_app_root_surface()); 

	button_size = hw_surface_get_size(button_image);
	button_size.width += 10;
	button_size.height += 10;

	ei_rect_t* rect_img = malloc(sizeof(ei_rect_t));
	*rect_img = hw_surface_get_rect(button_image);
	...
	ei_button_configure(button, &button_size, &button_color, &button_border_width, 
			    ...
			    NULL, &button_text_color, NULL, &button_image, &rect_img, &img_anchor, 
